...
addUser(obj).then((userList)=>{
  ...
  render() {
    ...
    const listOfUser=userdatamap.map((item)=><ListUser deleteUserHandler={this.deleteUserHandler} updateUserHandler={this.undateUser} key={item.id} item={item}/>)
     return(
          <div>
            <Header/>
            {isShowLoader && 
              <CircularProgress size={68} className="progressBar" />
            }
            {listOfUser}
            {modalDialog}
            <Button variant="fab" color="primary" aria-label="Add" className="add-user-btn" onClick={this.adduserBtnClick}>
                <AddIcon />
           </Button>
         </div>
    )
